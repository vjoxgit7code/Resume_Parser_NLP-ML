...
# sql connector
import sqlite3
connection =sqlite3.connect('USER_DATA8.db')
...
def insert_data(name, email, res_score, timestamp, no_of_page, reco_field, cand_level, skills, recommended_skills, courses):
    ...
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='USER_DATA8'")
    result = cursor.fetchone()
    if not result:
        cursor.execute("CREATE TABLE USER_DATA8(name TEXT, email TEXT, res_score TEXT, timestamp TEXT, no_of_page INT, reco_field TEXT, cand_level TEXT, skills TEXT, recommended_skills TEXT, courses TEXT)")
    cursor.execute("INSERT INTO USER_DATA8 (name, email, res_score, timestamp, no_of_page, reco_field, cand_level, skills, recommended_skills, courses) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (name, email, res_score, timestamp, no_of_page, reco_field, cand_level, skills, recommended_skills, courses))
    connection.commit()

 # inserting feedback data into  user_feedback table
def insertf_data(feed_name, feed_email, feed_score, comments, timestamp):
    cursor = connection.cursor()
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='USER_FEEDBACK8'")
    result = cursor.fetchone()
    if not result :
    # create the table
        cursor.execute("CREATE TABLE USER_FEEDBACK8(feed_name TEXT, feed_email TEXT, feed_score INTEGER, comments TEXT, timestamp TEXT)")
    cursor.execute("INSERT INTO USER_FEEDBACK8 (feed_name, feed_email, feed_score, comments, timestamp) VALUES (?, ?, ?, ?, ?)", (feed_name, feed_email, feed_score, comments, timestamp))
    connection.commit()
...
st.set_page_config(
   ...
   page_icon='F:\SRC\SRC\Logo\High-volume-blog-1024x536.png',
)
...
#(logo,Heading,Sidebar)    
    ...
    img = Image.open('F:\SRC\SRC\Logo\High-volume-blog-1024x536.png')
...
<p>Visitors <img src="https://counter9.stat.ovh/private/freecounterstat.php?c=t2xghr8ak6lfqt3kgru233378jya38dy" title="Free Counter" Alt="web counter" width="60px"  border="0" /></p>
    ...
    ###### CODE FOR FEEDBACK PAGE ######
        ...
        query = ' select * from USER_FEEDBACK8'
        ...
        cursor = connection.cursor()
        cursor.execute('SELECT feed_name, comments FROM USER_FEEDBACK8')
 ...
 ###### CODE FOR ADMIN SIDE (ADMIN) ######
    ...
    # Credentials
            if ad_user == 'ABCD' and ad_password == '123':
                ...
                # Display Data
                cursor = connection.cursor()
                cursor.execute('''SELECT * FROM USER_DATA8''')
                ...
                st.header("User 👨‍💻 Data")
                st.dataframe(df)
                st.markdown(get_table_download_link(df, 'USER_DATA8.csv', 'Download Report'), unsafe_allow_html=True)

                ## Admin Side Data
                query = 'select * from USER_DATA8;'
                plot_data = pd.read_sql(query, connection)
                print(plot_data.columns)

            # Display Data for feedback
                cursor = connection.cursor()
                cursor.execute('''SELECT * FROM USER_FEEDBACK8''')
                data = cursor.fetchall()
                ...
                ## FeedBack Side Data
                query1 = 'select * from USER_FEEDBACK8;'
